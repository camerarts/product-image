...
import { Upload, Loader2, Wand2, ArrowRight, FileText, Image as ImageIcon, Key, CheckCircle2, X, ChevronDown, Lock, LogIn, LogOut, User, Settings, Save, Shield } from 'lucide-react';
...
const UserWorkspace: React.FC = () => {
  ...
  const getEffectiveApiKey = (): string | undefined | null => {
    ...
    if (user) return undefined; // User logged in -> use system backend key
    return null; // Guest -> No Key
  };

  const handleAnalyze = async () => {
    const effectiveKey = getEffectiveApiKey();
    if (effectiveKey === null) {
      alert("非登录状态下，请先配置您的 API Key 或登录以使用系统资源");
  ...
  const parsePrompts = (text: string): ParsedPrompt[] => {
    ...
    sections.forEach((section, index) => {
      ...
      if (!title.includes('海报') && !title.includes('LOGO')) return;
      ...
      <div className="flex-1 h-screen overflow-hidden flex flex-col bg-white">
        ...
        <div className="h-16 border-b border-gray-200 flex items-center justify-between px-8 bg-white flex-shrink-0 z-20">
          <h2 className="font-bold text-lg text-gray-900 flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${user ? 'bg-green-500' : 'bg-gray-300'}`}></span>
            生成效果方案预览
          </h2>
             ...
             {!user ? (
               ...
               >
                ...
                <div className="flex items-center gap-3 bg-gray-50 rounded-full px-2 py-1 pr-4 border border-gray-100">
                   <div className={`w-7 h-7 rounded-full flex items-center justify-center text-white font-bold text-xs ${user.role === 'admin' ? 'bg-purple-600' : 'bg-indigo-600'}`}>
                     {user.username.substring(0, 2).toUpperCase()}
                   </div>
                   <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-gray-900 leading-tight">{user.username}</span>
                      <span className="text-[9px] text-gray-400 font-medium leading-tight flex items-center gap-1 uppercase">
                        {user.role}
                      </span>
                   </div>
                   {user.role === 'admin' && (
